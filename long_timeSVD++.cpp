...
inline void get_y_sum(int user, double * y_vector_sum)
{
    ...
    // neighborhood
    int size = neighborhood_sizes[user];
    for (int i = 0; i < size; i++)
    {
        ...
        for (int j = 0; j < K; j++)
        {
            y_vector_sum[j] += y[neighborhood_movie * (int)K + j]/sqrt(size);
        }
    }
}
...
inline void findQualPredictions()
{
    ...
    ofstream outputFile;
    ofstream probeFile;
    outputFile.open("long_timeSVD++_qual_output.dta");
    probeFile.open("long_timeSVD++_probe_output.dta");
...
void writeMatrices()
{
    ...
    ofstream user_file ("../long_user_matrix.txt");
    ...
    ofstream movie_file ("../long_movie_matrix.txt");
