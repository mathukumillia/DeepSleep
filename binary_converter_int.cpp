...
void initialize()
{
	...
	ratings = new int [((int) numPts) * POINT_SIZE];
	indices = new int [((int) numPts)];
}
...
void read_data()
{
	...
	fstream ratingsFile ("../um/allBaselineRemoved.dta");
	fstream indexFile ("../um/all.idx");
	// user, movie, date, rating
	int inputs [POINT_SIZE] = {};
	int index;
	int pt = 0;
...
void write_binary()
{
	...
	fstream ratings_file ("ratings_baseline_removed.bin", ios::out | ios::binary);
	fstream indices_file ("indices.bin", ios::out | ios::binary);

	ratings_file.write(reinterpret_cast<char *>(ratings), sizeof(int) * numPts * POINT_SIZE);
	indices_file.write(reinterpret_cast<char *>(indices), sizeof(int) * numPts);
